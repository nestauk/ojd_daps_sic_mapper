...
from data_getters import load_sic, load_json_dict, load_s3_data
import text_cleaning as tc
import sic_mapper_utils as su
    ...
    def __init__(
        ...
        if self.local:
            ...
            if not os.path.exists(self.data_dir):
                os.system(
                    f"aws s3 sync s3://prinz-green-jobs/data {self.data_dir}/green_industries"
                )
            else:
    ...
    def _fetch_data(self, file_name: str = None):
        ...
        if file_name.endswith(".json"):
            if self.local:
                ...
                data = load_s3_data("prinz-green-jobs", full_path)
            return data
        else:
